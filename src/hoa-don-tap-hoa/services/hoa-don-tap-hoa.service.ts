...
export class HoaDonTapHoaService {
  ...
  // Thống kê theo năm: mỗi tháng gồm tổng tiền hóa đơn (chỉ số thu) và số tiền
  // còn nợ (tongTien - đã thu qua các phiếu thu). Trả đủ 12 tháng, tháng không
  // có hóa đơn vẫn xuất hiện với giá trị 0 để FE vẽ biểu đồ theo năm.
  async statistics(req: StatisticsHoaDonTapHoaDto) {
    ...
    const items = await this.prisma.hoaDonTapHoa.findMany({
      where: { isDelete: false, ngayBan: { gte: from, lte: to } },
      select: {
        ngayBan: true,
        tongTien: true,
        phieuThuHdTh: { where: { isDelete: false }, select: { soTien: true } },
      },
    });
    ...
    for (const item of items) {
      ...
      const monthIndex = item.ngayBan.getUTCMonth();
      const tongTien = Number(item.tongTien ?? 0);
      const daThu = item.phieuThuHdTh.reduce((sum, pt) => sum + Number(pt.soTien ?? 0), 0);
      const conNo = tongTien - daThu;

      const cur = byMonth[monthIndex];
      cur.totalInvoices += 1;
      cur.totalRevenue += tongTien;
      cur.totalDebt += conNo;
